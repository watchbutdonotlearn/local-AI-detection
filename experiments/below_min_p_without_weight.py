...
def update_arrays_with_completion(
    ...
    for i in range(generated_prompt_token_length, len(tokens)):
        ...
        if suspiciousness[i] <= 0:
            ...
            #uncomment to get back extra weighting
...
def evaluate_analysis(dataset):
    ...
    for entry in dataset:
        ...
        if len(humanTexts) > 0:
            ...
            print('bottom 5 percent average for percent low (human):', average_bottom_5_percent(humanTexts))
            print('highest percent high in human text:', max(humanTextsHigh))
        if len(aiTexts) > 0:
            print('average percent low score of AI text:', sum(aiTexts) / len(aiTexts))
            print('highest percent low in AI text:', max(aiTexts))
            print('top 5 percent average for percent low (AI):', average_top_5(aiTexts))
            print('lowest percent high in AI text:', min(aiTextsHigh))
