...
def requestReversePrompt(api_link, generate_prompt_query, prompt_template):
    ...
    url = f'http://127.0.0.1:8002/v1/chat/completions'
...
def update_arrays_with_completion(
    ...
    percent_first_in_sentence_probzero = 0
    ...
    for i in range(generated_prompt_token_length, len(tokens)):
        ...
        try:
            test_variable = completion_data['completion_probabilities'][0]['top_probs']
        except:
        ...
        if suspiciousness[i] <= 0:
            ...
            chars = set('!.,:;?\n')
            if any((c in chars) for c in tokens[i - 1]):
                # print(tokens[i - 2], tokens[i - 1], tokens[i], 'is first token in sentence part (likely human). upping the weight.')
                # number_suspicious_low += 1
        ...
        #     chars = set('!.,:;?\n')
        #     if any((c in chars) for c in tokens[i - 1]):
        #         # print(tokens[i - 2], tokens[i - 1], tokens[i], 'is first token in sentence part (likely AI), applying negative weight to num_suspicious_low.')
        #         number_suspicious_low = number_suspicious_low - .5
        # else:
        #     chars = set('!.,:;?\n')
        #     if any((c in chars) for c in tokens[i - 1]):
        #         # print(tokens[i - 2], tokens[i - 1], tokens[i], 'is first token in sentence part (not prob 0 or 10), applying small negative weight to num_suspicious_low.')
        #         number_suspicious_low = number_suspicious_low - .1
...
def run_analysis(inputText, answer):
    generate_prompt_query = [{'role': 'user', 'content': f'You will write a prompt for a language model that will output the following text. Use your own original words. Make sure to include the tone and style of the text. Only output the prompt.\n\n{inputText}'}]
    ...
    print('chosen probs list:')
    print(trimmed_chosen_probs)

    print('suspiciousness list:')
    print(trimmed_suspiciousness)

    isHuman = False

    if percent_low > humanThreshold:
        isHuman = True
...
def evaluate_analysis(dataset):
    ...
    for entry in dataset:
        ...
        # print("detector verdict is_human: ", analysis_result)
        # print("is_correct:", is_correct)
        # print("percent_first_in_sentence_probzero: ", percent_first_in_sentence_probzero)

        # Append the result to the results array
        #percent low, expected answer, is correct, percent high
        if len(entry['input_text'].split()) > 75:
            print('length is above 75 words. Continuing.')
            ...
            print('length is below 75 words, NOT ADDING TO FINAL RESULTS')
            print('-----------------------------------------------------------------------------------------------')
            continue
        ...
        if not is_correct:
            ...
            if expected_answer == True: #is human
                numFalseHuman += 1
            if expected_answer == False:
                numFalseAI += 1

        print("number of false positive human: ", numFalseHuman, "\nnumber of false positive AI: ", numFalseAI)
